...
* ok関数は, 1つ目の引数の式が真ならok, 負ならngとなります.
...
* 実装した関数に問題があり, 条件式が負になる場合, このようなエラーが出力されます.
...
* `$self->render(text => 'Hello, world!');`で, ｢'text/html'の形式で, 'hello, world!'と出力せよ!｣という命令をしています.
...
* 最後に`=> 'index';`と記載することで, 使用するテンプレートを指定します.
